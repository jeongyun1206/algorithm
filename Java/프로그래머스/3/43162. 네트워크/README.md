...
네트워크의 정의는 그래프의 정의와 같다. 즉, 몇개의 그래프가 존재하는지를 구하면 풀리는 문제이다.

1. 0번 노드부터 n-1번 노드까지 순차적으로 탐색한다.
   1-1. 방문한 기록이 있으면 1번으로 돌아간다.
   1-2. 방문한 기록이 없으면 카운트를 올리고 2번을 실행한다.
2. 방문 노드를 대상으로 dfs를 실행하여 순회한 노드를 모두 방문으로 기록한다.
3. n-1번째 노드까지 반복한 뒤 카운트를 반환한다.
